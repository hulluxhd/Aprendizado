...
let pessoasArray = []
let pessoasString = localStorage.getItem('pessoas')


if (pessoasString != null) {
    pessoasArray = pessoasString.split('\n')

    for (pessoa in pessoasArray){
        let pessoaObj = JSON.parse(pessoa)
        ol.innerHTML += `<li>nome: ${pessoaObj.value}, sobrenome: ${pessoaObj.value}</li>`
}} else
    console.log('vazio')
...
btn.addEventListener('click', event => {
    ...
    let pessoasStringify = JSON.stringify(pessoasObj)

    pessoasArray.push(pessoasStringify)
